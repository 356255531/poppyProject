    ...
    def compute_expectation(self, curr_state, freq_dict):
        ...
        for action in self.state_action_space.get_eligible_actions(curr_state):
            ...
            for state in self.states:
                ...
                expectation_component = self.rewardObj.get_rewards(curr_state, action, nextState=state, problemType='capture') + self.gamma * self.values[self.states.index(state)]
                exp_curr_action += probab_currState_action_state * expectation_component
